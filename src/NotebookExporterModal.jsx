...
const NotebookExporterModal = ({ isOpen, onClose, trackData, storyboardData }) => {
  ...
  // Compile Pure Storytelling Markdown Dossier (Zero Timestamps)
  const buildDossierMarkdown = () => {
    ...
    // 1. Narrative Premise & Poetic Core
    ...
    if (storyboardData?.songOverview) {
      ...
      md += `* **Core Theme:** A deep-dive cinematic exploration of the song's emotional tension, character dilemma, and poetic metaphors.\n\n`;
    }

    // 2. Distilled Story Lyrics (Clean Narrative Beats)
...
1. Deconstruct the Core Emotion: Translate the lyrics into pure human experience, emotional tension, and the singer's unspoken dilemma.
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm font-sans">
      <div className="relative w-full max-w-lg bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl overflow-hidden p-5 space-y-4 animate-in fade-in zoom-in-95 duration-150">
        ...
        <div className="flex items-center justify-between border-b border-slate-800 pb-3">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-400">Pure storytelling dossier for NotebookLM, Imagen 3 & Podcast Studio</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        {/* Step 2: Master Storytelling Dossier */}
        <div className="space-y-1.5 bg-slate-950 p-3 rounded-xl border border-slate-800">
          <label className="text-[10px] font-bold uppercase tracking-wider text-slate-400 flex items-center gap-1">
            <Film size={11} className="text-amber-400" />
            <span>2. Master Storytelling Dossier</span>
          </label>
          <div className="flex items-center justify-between gap-2">
        ...
        <div className="space-y-1.5 bg-slate-950 p-3 rounded-xl border border-slate-800">
          ...
          <div className="flex items-center justify-between gap-2">
            <span className="text-xs text-slate-300">2-Host Narrative & Visual Deep-Dive Prompt</span>
            <button
