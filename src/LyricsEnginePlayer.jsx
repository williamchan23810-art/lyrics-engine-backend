...
const LyricsEnginePlayer = ({ trackData: initialTrackData, apiBaseUrl = '' }) => {
  ...
  // Determine active lyrics array based on Distillation mode
  const rawLyrics = trackData?.lyrics || [];
  const activeLyrics = isDistilled ? distillLyrics(rawLyrics) : rawLyrics;

  const handleToggleDistillation = () => {
    setIsDistilled((prev) => !prev);
  };
  ...
  const handleExportToNotebook = async () => {
    const success = await copyToNotebookClipboard(
      { ...trackData, lyrics: activeLyrics },
      storyboardData
    );
    if (success) {
  ...
  const handleSongAutoGenerated = (data) => {
    setTrackData(data);
    setIsDistilled(false);
    ...
    <div className="h-screen w-screen bg-slate-950 text-slate-100 flex flex-col overflow-hidden font-sans">
      ...
      <header className="flex flex-wrap items-center justify-between p-3.5 bg-slate-900/90 border-b border-slate-800 backdrop-blur-md gap-2 z-20 shrink-0">
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          ...
          {/* Lyrics Distillation Button */}
          <button
            onClick={handleToggleDistillation}
          ...
          >
            ...
            <span>{isDistilled ? 'Show Raw (All Lines)' : 'Distill Lyrics'}</span>
          </button>
      ...
      {/* Modals (Storyboard automatically receives the distilled lyrics) */}
      <AutoGeneratorModal 
